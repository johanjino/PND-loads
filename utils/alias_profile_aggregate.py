...
for bench in benches:
    ...
    for run in ["0", "1", "2", ""]:
        ...
        raw_results_dir = "/work/johan/results/profiling/must_alias_count/a14/" + bench.split(".")[1].rstrip("_s") + "." + run + "/raw/"
            ...
            if os.path.isdir(dirname) and dirname[0].isdigit() and dirname.split('.')[1] == 'out':
                ...
                for load in loads:
                    if load not in aggregated_loads:
                        ...
                        for cycle, count in loads[load]["alias_map"].items():
                            if store in aggregated_loads[load]["alias_map"]:
                                aggregated_loads[load]["alias_map"][store] += count
                            else:
                                aggregated_loads[load]["alias_map"][store] = count

        output_profile_info = process_load_map(aggregated_loads, binary)

        filename = "/work/johan/spec2017/benchspec/CPU/"+bench+"/run/"+"AliasPGOInfoShift4Train."+run
