...
p = Popen(gem5+"build/ARM/gem5.fast --outdir=checkpoints."+str(run)+".adjusted "+gem5+"configs/example/se.py --cpu-type=NonCachingSimpleCPU --restore-with-cpu=NonCachingSimpleCPU -r 1 --simpoint-offset "+offset+" --last-simpoint "+chkpt+" --checkpoint-dir checkpoints."+str(run)+" --take-simpoint-checkpoint=/sim_home/luke/simpoints/"+bench+"."+str(run)+".simpts.adjusted,/sim_home/luke/simpoints/"+bench+"."+str(run)+".weights.adjusted,100000000,10000000 -c "+command.split()[0]+" --options=\""+' '.join(command.split()[1:])+"\" --mem-size=50GB", shell=True)
p.wait()
