...
for addr_type in addr_types:
    ...
    for line in stats:
        ...
        if name == field:
            results[current_benchmark][addr_type] = float(value)

fig, ax = plt.subplots()
fig.set_size_inches(12,12,forward=True)
bar_width = 0.25
for c, addr_type in enumerate(addr_types):
    bars = ax.bar(np.arange(len(benchmark_names))+(bar_width*c)-(bar_width/len(addr_types)), [results[name][addr_type] for name in benchmark_names], width=bar_width)
    for bar in bars:
        value = bar.get_height()
        if abs(round(value,1)) > 0.1:
            if value > 0: value += 0.025
            elif value < 0: value -= 0.025
            ax.text(bar.get_x() + bar.get_width() / 2+(bar_width/len(addr_types)) - 0.05, value, str(round(value,1))+"%", ha='center', va='bottom' if value >=0 else 'top', rotation=90)

ax.legend(labels=addr_types, fontsize=16)#, loc='lower right')
...
ax.set_title('CPI Difference - '+run_type.replace('_',' '), fontsize=18)
ax.set_ylim(top=4, bottom=-13)
plt.tight_layout()
plt.savefig("/work/muke/results/"+run_type+"/graphs/"+cpu_model+".png", dpi=300)
