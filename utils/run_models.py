...
cpu_model_dir = "/work/johan/PND-Loads/cpu_models/"

gem5_dir = "/work/johan/PND-Loads/gem5/"
...
if 'base' in addr_types:
    for model in cpu_models:
        ...
        scons = subprocess.run("scons build/ARM/gem5.fast -j 10 --with-lto --ignore-style --linker=gold", shell=True, check=True)
        run = subprocess.run("python3 /work/johan/PND-Loads/utils/run_all_chkpts.py "+run_type+" base "+model+" with_base "+benches , shell=True, check=True)
...
for model in cpu_models:
    ...
    #    run = subprocess.run("python3 /work/johan/PND-Loads/utils/run_all_chkpts.py "+run_type+" "+addr_type+" "+model+" without_base "+benches, shell=True, check=True)