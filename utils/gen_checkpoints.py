...
subprocess.run(gem5+"build/ARM/gem5.fast --outdir="+checkpoint_path+"/checkpoints."+str(run)+" "+gem5+"configs/deprecated/example/se.py --cpu-type=NonCachingSimpleCPU --take-simpoint-checkpoint=/work/muke/simpoints-expanded/"+bench_name+".simpts,/work/muke/simpoints-expanded/"+bench_name+".weights,100000000,10000000 -c "+command.split()[0]+" --options=\""+' '.join(command.split()[1:])+"\" --mem-size=50GB 2>&1 > "+bench_name+".out 2>&1", shell=True, check=True)
