...
static cl::opt<bool>
    EnableLoadAliasMetadataInsertion("aarch64-enable-load-alias-metadata-insertion", cl::Hidden,
                                     cl::desc("Enable the pass that scans for IR"
                                              " metadata on GEPs and replaces"
                                              " loads that use them with special"
                                              " opcodes that encode alias behavior"),
                                     cl::init(true));
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeAArch64Target() {
  ...
  initializeAArch64LoadAliasMetadataInsertionPass(*PR);
...
void AArch64PassConfig::addPreEmitPass2() {
  ...
  //if (TM->getOptLevel() != CodeGenOpt::None && EnableLoadAliasMetadataInsertion)
    addPass(createAArch64LoadAliasMetadataInsertion());
}
