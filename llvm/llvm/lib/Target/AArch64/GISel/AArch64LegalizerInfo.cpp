...
AArch64LegalizerInfo::AArch64LegalizerInfo(const AArch64Subtarget &ST)
  ...
  auto IsPtrVecPred = [=](const LegalityQuery &Query) {
    ...
    unsigned AddrSpace;
    if (EltTy.isPointer())
       AddrSpace = EltTy.getAddressSpace();
    else
      return false;
    return EltTy.isPointer() && (AddrSpace == 0
                                 || AddrSpace == PREDICT_ALIAS_ADDRESS_SPACE
                                 || AddrSpace == PREDICT_NO_ALIAS_ADDRESS_SPACE);
  };
...
bool AArch64LegalizerInfo::legalizeLoadStore(
  ...
  if (!ValTy.isVector() || !ValTy.getElementType().isPointer()){
    const unsigned AddrSpace = ValTy.getAddressSpace();
    if (AddrSpace != 0 || AddrSpace != PREDICT_ALIAS_ADDRESS_SPACE
        || AddrSpace != PREDICT_NO_ALIAS_ADDRESS_SPACE){
